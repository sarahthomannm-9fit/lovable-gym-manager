...
export function AulasExperimentais() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addAulaExperimental({
      ...
      data_conversao: null,
      plano_convertido_id: null,
      avaliacao_experiencia: null,
      feedback: null,
      atendido_por: null,
    });
  ...
  const handleConvert = async () => {
    if (selectedAula && selectedPlano) {
      await converterParaAluno(selectedAula.id, selectedPlano);
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setFormData(prev => ({ ...prev, fonte: value as AulaExperimental['fonte'] }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, horario_agendado: e.target.value }))}
                  />
                </div>
              </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold">{metricas.agendadas + metricas.confirmadas}</span>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold">{metricas.convertidas}</span>
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold">{metricas.taxaComparecimento}%</span>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {Object.entries(porFonte).map(([fonte, quantidade]) => (
              <Badge key={fonte} variant="outline" className="text-sm py-1 px-3">
                {fontes.find(f => f.value === fonte)?.label || fonte}: {quantidade}
              </Badge>
            ))}
          </div>
        </CardContent>
              ...
              <CardContent className="space-y-3">
                ...
                {aula.fonte && (
                  <Badge variant="outline">
                    {fontes.find(f => f.value === aula.fonte)?.label || aula.fonte}
                  </Badge>
                )}
                ...
                {aula.avaliacao_experiencia && (
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i} 
                        className={`w-4 h-4 ${i < aula.avaliacao_experiencia! ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'}`} 
                      />
                    ))}
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-2 flex-wrap">
                  {aula.status === 'agendada' && (
                    <>
                      <Button 
                        size="sm" 
                        variant="outline"
                        onClick={() => updateAulaExperimental(aula.id, { status: 'confirmada' })}
                      >
                        Confirmar
                      </Button>
                      <Button 
                        size="sm" 
                        variant="destructive"
                        onClick={() => updateAulaExperimental(aula.id, { status: 'cancelada' })}
                      >
                        Cancelar
                      </Button>
                    </>
                  ...
                  {(aula.status === 'confirmada' || aula.status === 'agendada') && (
                    <>
                      <Button 
                        size="sm"
                        onClick={() => updateAulaExperimental(aula.id, { status: 'realizada' })}
                      >
                      ...
                      <Button 
                        size="sm" 
                        variant="secondary"
                        onClick={() => updateAulaExperimental(aula.id, { status: 'nao_compareceu' })}
                      >
                  ...
                  {aula.status === 'realizada' && (
                    <>
                      ...
                      <Button 
                        size="sm" 
                        variant="secondary"
                        onClick={() => updateAulaExperimental(aula.id, { status: 'nao_convertida' })}
                      >
                        Não Converteu
                      </Button>
                    </>
      ...
      <Dialog open={convertDialogOpen} onOpenChange={setConvertDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Converter para Aluno</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p>
              Converter <strong>{selectedAula?.nome}</strong> para aluno:
            </p>
            <div>
                ...
                <SelectContent>
                  {plans.filter(p => p.ativo).map(plano => (
                    <SelectItem key={plano.id} value={plano.id}>
                      {plano.nome} - {plano.preco.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleConvert} disabled={!selectedPlano}>
                Confirmar Conversão
              </Button>
            </div>
          </div>
