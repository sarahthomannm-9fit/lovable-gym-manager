...
export default function ResidentEntry() {
  ...
  const start = async () => {
    ...
    if (error) toast.error(error.message); else { sessionStorage.setItem('9fit:onboarding:aluno', data); const { error: authError } = await supabase.auth.signInWithOtp({ email: form.email.trim(), options: { emailRedirectTo: `${window.location.origin}/morador/onboarding` } }); if (authError) toast.error(authError.message); else toast.success('Enviamos um link de acesso para seu e-mail.'); }
    setSaving(false);
  };
  return <div className="min-h-screen bg-background p-4 sm:p-8"><Card className="mx-auto mt-8 max-w-lg rounded-sm shadow-elegant"><CardHeader><p className="text-sm font-medium text-primary">Nine Living · Saúde que conecta</p><CardTitle className="font-display text-3xl font-normal">{contexto === 'evento' ? 'Confirme sua participação' : contexto === 'campanha' ? 'Comece sua jornada' : 'Comece seu onboarding'}</CardTitle><p className="text-sm text-muted-foreground">Escolha seu condomínio e responda algumas perguntas para receber um treino adequado.</p></CardHeader><CardContent className="space-y-4"><div><Label>Condomínio</Label><Select value={organizationId} onValueChange={setOrganizationId}><SelectTrigger><SelectValue placeholder="Escolha seu condomínio" /></SelectTrigger><SelectContent>{organizations.map((o) => <SelectItem key={o.id} value={o.id}>{o.nome}</SelectItem>)}</SelectContent></Select></div><div><Label>Nome</Label><Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} /></div><div><Label>E-mail</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div><div><Label>Unidade (opcional)</Label><Input value={form.unidade} onChange={(e) => setForm({ ...form, unidade: e.target.value })} /></div><Button className="w-full" disabled={saving} onClick={start}>{saving ? 'Iniciando…' : 'Continuar para triagem'}</Button></CardContent></Card></div>;
}
