...
export default function Events() {
  ...
  useEffect(() => { (async () => { setLoading(true); const { data: u } = await supabase.auth.getUser(); if (!u.user) return; const { data: a } = await (supabase as any).from('alunos').select('id, organization_id').eq('user_id', u.user.id).maybeSingle(); if (!a) return; setAlunoId(a.id); const { data } = await (supabase as any).from('health_day_events').select('*').eq('organization_id', activeOrg?.id || a.organization_id).eq('status', 'publicado').order('inicio'); setEvents(data || []); setLoading(false); })(); }, [activeOrg?.id]);
