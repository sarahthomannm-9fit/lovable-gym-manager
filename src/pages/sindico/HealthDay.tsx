...
export default function HealthDay() {
  ...
  const updateRegistration = async (id: string, status: string) => { const { error } = await (supabase as any).from('health_day_registrations').update({ status }).eq('id', id); if (error) toast.error(error.message); else { toast.success('Inscrição atualizada.'); if (selectedEvent) showParticipants(selectedEvent); load(); } };
  ...
  return <div className="min-h-full bg-background p-6 space-y-6"><div><h1 className="font-display text-3xl font-normal tracking-tight text-foreground">Health Day</h1><p className="text-sm text-muted-foreground">Crie eventos de saúde, defina vagas e acompanhe a ativação do condomínio.</p></div>
    <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Novo evento</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><div><Label>Nome</Label><Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} /></div><div><Label>Vagas</Label><Input type="number" min="1" value={form.capacidade} onChange={(e) => setForm({ ...form, capacidade: e.target.value })} /></div><div><Label>Início</Label><Input type="datetime-local" value={form.inicio} onChange={(e) => setForm({ ...form, inicio: e.target.value })} /></div><div><Label>Fim</Label><Input type="datetime-local" value={form.fim} onChange={(e) => setForm({ ...form, fim: e.target.value })} /></div><div className="sm:col-span-2"><Label>Descrição</Label><Textarea value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} /></div><div className="sm:col-span-2"><Button onClick={create}>Criar Health Day</Button></div></CardContent></Card>
    <div className="grid gap-4">{events.map((event) => <Card key={event.id} className="rounded-sm shadow-elegant"><CardContent className="flex flex-wrap items-center justify-between gap-4 p-5"><div><h2 className="font-semibold">{event.nome}</h2><p className="text-sm text-muted-foreground">{new Date(event.inicio).toLocaleString('pt-BR')} · {event.capacidade} vagas</p><div className="flex gap-2 text-xs"><Badge variant="outline">{event.inscritos || 0} inscritos</Badge><Badge variant="outline">{event.espera || 0} espera</Badge><Badge variant="outline">{event.checkins || 0} check-ins</Badge><Badge variant="outline">{Math.max(0, event.capacidade - (event.inscritos || 0))} vagas restantes</Badge></div></div><div className="flex items-center gap-2"><Badge variant="outline">{event.status}</Badge><Button size="sm" variant="outline" onClick={() => showParticipants(event.id)}>Participantes</Button><Button size="sm" variant="ghost" onClick={() => { const link = `${window.location.origin}/morador/eventos?event=${event.id}`; navigator.clipboard.writeText(link); toast.success("Link do Health Day copiado."); }}>Copiar QR/link</Button>{event.status === 'rascunho' && <Button size="sm" onClick={() => publish(event.id)}>Publicar</Button>}</div></CardContent></Card>)}{loading ? <p className="text-sm text-muted-foreground">Carregando Health Days...</p> : !events.length && <p className="text-sm text-muted-foreground">Nenhum Health Day criado.</p>}{selectedEvent && <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>Participantes</CardTitle><Button size="sm" variant="outline" onClick={exportParticipants}>Exportar CSV</Button></div></CardHeader><CardContent className="space-y-2">{participants.map((p) => <div key={p.id} className="flex items-center justify-between border-b py-2 text-sm"><span>{p.alunos?.nome || 'Morador'}<span className="block text-xs text-muted-foreground">{p.alunos?.email || ''}</span></span><div className="flex items-center gap-2"><Badge variant={p.status === 'checkin' ? 'default' : 'outline'}>{p.status}</Badge>{p.status === 'lista_espera' && <Button size="sm" onClick={() => updateRegistration(p.id, 'confirmado')}>Promover</Button>}{['inscrito','confirmado','lista_espera'].includes(p.status) && <Button size="sm" variant="ghost" onClick={() => updateRegistration(p.id, 'cancelado')}>Cancelar</Button>}</div></div>)}{!participants.length && <p className="text-sm text-muted-foreground">Nenhum participante.</p>}</CardContent></Card>}</div>
  </div>;
}
