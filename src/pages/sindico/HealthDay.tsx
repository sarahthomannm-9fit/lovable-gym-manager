...
export default function HealthDay() {
  ...
  return <div className="p-6 space-y-6"><div><h1 className="text-2xl font-bold">Health Day</h1><p className="text-sm text-muted-foreground">Crie eventos de saúde, defina vagas e acompanhe a ativação do condomínio.</p></div>
    ...
    <div className="grid gap-4">{events.map((event) => <Card key={event.id}><CardContent className="flex flex-wrap items-center justify-between gap-4 p-5"><div><h2 className="font-semibold">{event.nome}</h2><p className="text-sm text-muted-foreground">{new Date(event.inicio).toLocaleString('pt-BR')} · {event.capacidade} vagas</p><div className="flex gap-2 text-xs"><Badge variant="outline">{event.inscritos || 0} inscritos</Badge><Badge variant="outline">{event.espera || 0} espera</Badge><Badge variant="outline">{event.checkins || 0} check-ins</Badge><Badge variant="outline">{Math.max(0, event.capacidade - (event.inscritos || 0))} vagas restantes</Badge></div></div><div className="flex items-center gap-2"><Badge variant="outline">{event.status}</Badge>{event.status === 'rascunho' && <Button size="sm" onClick={() => publish(event.id)}>Publicar</Button>}</div></CardContent></Card>)}{!events.length && <p className="text-sm text-muted-foreground">Nenhum Health Day criado.</p>}</div>
  </div>;
}
