...
export default function OrganizationsAdmin() {
  ...
  useEffect(() => { if (selected) { loadMembers(selected.id); loadFacilities(selected.id); (async () => { const { data } = await (supabase as any).from('organization_invites').select('id,email,papel,status,expires_at,created_at').eq('organization_id', selected.id).order('created_at', { ascending: false }); setInvites(data || []); })(); } }, [selected]);
  ...
  const confirmImport = async () => { if (!selected || !importRows.length) return; const { data, error } = await (supabase as any).rpc('bulk_import_residents', { p_organization_id: selected.id, p_rows: importRows }); if (error) return toast.error(error.message); toast.success(`${data?.importados || 0} moradores importados.`); setImportRows([]); };
  ...
  return (
    <div className="min-h-full bg-background p-6 space-y-6">
      ...
      {selected && (<>
        ...
        <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Importar moradores</CardTitle><p className="text-sm text-muted-foreground">CSV com colunas nome, email e unidade.</p></CardHeader><CardContent className="space-y-3"><Input type="file" accept=".csv,text/csv" onChange={(e) => { const file = e.target.files?.[0]; if (file) void importCsv(file); }} />{importRows.length > 0 && <div className="space-y-2"><p className="text-sm">{importRows.length} linhas prontas para importar.</p><div className="max-h-32 overflow-y-auto rounded-sm border p-2 text-xs">{importRows.slice(0, 5).map((row, index) => <div key={index}>{row.nome || '—'} · {row.email || 'sem e-mail'} · {row.unidade || '—'}</div>)}</div><Button onClick={confirmImport}>Confirmar importação</Button></div>}</CardContent></Card>

        <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Convites do condomínio</CardTitle></CardHeader><CardContent>{!invites.length ? <p className="text-sm text-muted-foreground">Nenhum convite enviado.</p> : <div className="space-y-2">{invites.map((invite) => <div key={invite.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-border/40 pb-2 text-sm"><span>{invite.email}<span className="block text-xs text-muted-foreground">{invite.papel} · expira {new Date(invite.expires_at).toLocaleDateString('pt-BR')}</span></span><div className="flex items-center gap-2"><Badge variant="outline">{invite.status}</Badge>{invite.status === "pendente" && <Button size="sm" variant="ghost" onClick={async () => { const { error } = await (supabase as any).rpc("cancel_organization_invite", { p_invite_id: invite.id }); if (error) toast.error(error.message); else { toast.success("Convite cancelado."); if (selected) { const { data } = await (supabase as any).from("organization_invites").select("id,email,papel,status,expires_at,created_at").eq("organization_id", selected.id).order("created_at", { ascending: false }); setInvites(data || []); } } }}>Cancelar</Button>}</div></div>)}</div>}</CardContent></Card>
