...
export default function OrganizationsAdmin() {
  ...
  const [residentForm, setResidentForm] = useState({ nome: '', email: '', unidade: '', telefone: '' });
  ...
  const addResident = async () => { if (!selected || !residentForm.nome.trim() || !residentForm.email.includes('@')) return toast.error('Informe nome e e-mail válidos do morador.'); const { error: residentError } = await (supabase as any).rpc('bulk_import_residents', { p_organization_id: selected.id, p_rows: [{ nome: residentForm.nome.trim(), email: residentForm.email.trim(), unidade: residentForm.unidade.trim(), telefone: residentForm.telefone.trim() }] }); if (residentError) return toast.error(residentError.message); const { data, error } = await (supabase as any).rpc('create_organization_invite', { p_organization_id: selected.id, p_email: residentForm.email.trim(), p_papel: 'user' }); if (error) return toast.error(error.message); setResidentForm({ nome: '', email: '', unidade: '', telefone: '' }); toast.success(`Morador cadastrado. Convite criado para ${data?.email || residentForm.email.trim()}.`); };

  const sendInvite = async () => { if (!selected || !inviteEmail.trim()) return toast.error('Informe o e-mail.'); const { data, error } = await (supabase as any).rpc('create_organization_invite', { p_organization_id: selected.id, p_email: inviteEmail.trim(), p_papel: inviteRole }); if (error) return toast.error(error.message); const link = `${window.location.origin}/convite/${data?.token}`; setInviteLink(link); const delivery = await supabase.functions.invoke('send-organization-invite', { body: { inviteId: data?.id, token: data?.token, email: inviteEmail.trim(), papel: inviteRole } }); if (delivery.error) toast.warning('Convite criado, mas o e-mail não foi enviado. Copie o link manualmente.'); else if ((delivery.data as any)?.sent) toast.success('Convite criado e e-mail enviado.'); else toast.success('Convite criado. Copie o link enquanto o envio de e-mail não está configurado.'); setInviteEmail(''); };
  ...
  return (
    <div className="min-h-full bg-background p-6 space-y-6">
      ...
      {selected && (<>
        ...
        <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Cadastrar morador</CardTitle><p className="text-sm text-muted-foreground">Cadastre uma pessoa e gere o acesso sem criar usuário previamente.</p></CardHeader><CardContent className="grid gap-2 sm:grid-cols-5 items-end"><div><Label>Nome</Label><Input value={residentForm.nome} onChange={(e) => setResidentForm({ ...residentForm, nome: e.target.value })} /></div><div><Label>E-mail</Label><Input type="email" value={residentForm.email} onChange={(e) => setResidentForm({ ...residentForm, email: e.target.value })} /></div><div><Label>Unidade</Label><Input value={residentForm.unidade} onChange={(e) => setResidentForm({ ...residentForm, unidade: e.target.value })} /></div><div><Label>Telefone</Label><Input value={residentForm.telefone} onChange={(e) => setResidentForm({ ...residentForm, telefone: e.target.value })} /></div><Button onClick={addResident}>Cadastrar e convidar</Button></CardContent></Card>
        ...
        <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Convites do condomínio</CardTitle><p className="text-sm text-muted-foreground">Acompanhe o acesso e a entrega de cada convite.</p></CardHeader><CardContent>{!invites.length ? <p className="text-sm text-muted-foreground">Nenhum convite enviado.</p> : <div className="space-y-2">{invites.map((invite) => { const delivery = invite.organization_invite_deliveries?.[0]; return <div key={invite.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-border/40 pb-2 text-sm"><span>{invite.email}<span className="block text-xs text-muted-foreground">{invite.papel} · expira {new Date(invite.expires_at).toLocaleDateString('pt-BR')} · entrega: {delivery?.status || 'não enviada'}</span></span><div className="flex items-center gap-2">{invite.status === "pendente" && <Button size="sm" variant="ghost" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/convite/${invite.token}`); toast.success("Link copiado."); }}>Copiar link</Button>}<Badge variant="outline">{invite.status}</Badge>{invite.status !== "pendente" && <Button size="sm" variant="ghost" onClick={async () => { const { error } = await (supabase as any).rpc("resend_organization_invite", { p_invite_id: invite.id }); if (error) toast.error(error.message); else toast.success("Novo convite criado."); }}>Reenviar</Button>}{invite.status === "pendente" && <Button size="sm" variant="ghost" onClick={async () => { const { error } = await (supabase as any).rpc("cancel_organization_invite", { p_invite_id: invite.id }); if (error) toast.error(error.message); else { toast.success("Convite cancelado."); if (selected) { const { data } = await (supabase as any).from("organization_invites").select("id,email,papel,status,expires_at,created_at,organization_invite_deliveries(status,created_at)").eq("organization_id", selected.id).order("created_at", { ascending: false }); setInvites(data || []); } } }}>Cancelar</Button>}</div></div>; })}</div>}</CardContent></Card>
