  ...
  return (
    <div className="min-h-full bg-background p-6 space-y-6">
      ...
      {selected && (<>
        ...
        <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Entrada do condomínio</CardTitle><p className="text-sm text-muted-foreground">Use este endereço para gerar o QR Code da academia.</p></CardHeader><CardContent><div className="flex gap-2"><Input readOnly value={selected ? `${window.location.origin}/entrada?organization=${selected.id}` : ''} /><Button variant="outline" onClick={() => { if (selected) { navigator.clipboard.writeText(`${window.location.origin}/entrada?organization=${selected.id}`); toast.success("Link de entrada copiado."); } }}>Copiar link</Button></div></CardContent></Card>
