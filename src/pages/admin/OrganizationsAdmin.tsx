  ...
  return (
    <div className="min-h-full bg-background p-6 space-y-6">
      ...
      {selected && (<>
        ...
        <Card className="rounded-sm shadow-elegant"><CardHeader><CardTitle>Convites do condomínio</CardTitle></CardHeader><CardContent>{!invites.length ? <p className="text-sm text-muted-foreground">Nenhum convite enviado.</p> : <div className="space-y-2">{invites.map((invite) => <div key={invite.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-border/40 pb-2 text-sm"><span>{invite.email}<span className="block text-xs text-muted-foreground">{invite.papel} · expira {new Date(invite.expires_at).toLocaleDateString('pt-BR')}</span></span><div className="flex items-center gap-2">{invite.status === "pendente" && <Button size="sm" variant="ghost" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/convite/${invite.token}`); toast.success("Link copiado."); }}>Copiar link</Button>}<Badge variant="outline">{invite.status}</Badge>{invite.status === "pendente" && <Button size="sm" variant="ghost" onClick={async () => { const { error } = await (supabase as any).rpc("cancel_organization_invite", { p_invite_id: invite.id }); if (error) toast.error(error.message); else { toast.success("Convite cancelado."); if (selected) { const { data } = await (supabase as any).from("organization_invites").select("id,email,papel,status,expires_at,created_at").eq("organization_id", selected.id).order("created_at", { ascending: false }); setInvites(data || []); } } }}>Cancelar</Button>}</div></div>)}</div>}</CardContent></Card>
