...
export function SupabasePayments() {
  ...
  // Smart prefill when student is selected
  useEffect(() => {
  ...
  const handleBatchGenerate = async () => {
    ...
    try {
      ...
      // Check existing payments for this month to avoid duplicates
      const existingStudentIds = new Set(
        payments
          .filter(p => p.referencia_mes === refMonth)
          .map(p => p.aluno_id)
      );
      ...
      if (toCreate.length === 0) {
        toast({ title: "Info", description: "Todas as cobranças do mês já foram geradas." });
      ...
      for (const student of toCreate) {
        ...
        await addPayment({
          ...
          metodo_pagamento: student.forma_pagamento || 'pix',
          status: 'pendente',
          referencia_mes: refMonth
        });
      }
      toast({ title: "🎉 Cobranças geradas!", description: `${toCreate.length} cobranças criadas para o mês.` });
    } catch (error) {
      console.error('Batch generation error:', error);
      toast({ title: "Erro", description: "Falha ao gerar cobranças em lote", variant: "destructive" });
    } finally {
      setBatchLoading(false);
    }
  };
  ...
  const handleMarkAsPaid = async (paymentId: string) => {
    try {
      await updatePayment(paymentId, { status: 'pago', data_pagamento: new Date().toISOString().split('T')[0] });
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "cancelado": case "atrasado": return "bg-red-100 text-red-800";
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case "pago": return "Pago"; case "pendente": return "Pendente";
      case "cancelado": return "Cancelado"; case "atrasado": return "Atrasado";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleBatchGenerate} disabled={batchLoading}>
            <Zap className="w-4 h-4 mr-2" />
            {batchLoading ? 'Gerando...' : 'Gerar Cobranças do Mês'}
          </Button>
          <Dialog>
            <DialogTrigger asChild>
              <Button><Plus className="w-4 h-4 mr-2" />Nova Cobrança</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  <Label>Aluno</Label>
                  <Select value={newPayment.studentId} onValueChange={(value) => setNewPayment(prev => ({...prev, studentId: value}))}>
                    <SelectTrigger><SelectValue placeholder="Selecione o aluno" /></SelectTrigger>
                    <SelectContent>
                      {students.map(student => (
                        <SelectItem key={student.id} value={student.id}>
                          {student.nome}
                          {student.valor_mensalidade ? ` (R$ ${student.valor_mensalidade})` : ''}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div>
                  <Label>Valor (R$)</Label>
                  <Input type="number" step="0.01" value={newPayment.amount} onChange={(e) => setNewPayment(prev => ({...prev, amount: e.target.value}))} placeholder="0,00" />
                </div>
                <div>
                  <Label>Data de Vencimento</Label>
                  <Input type="date" value={newPayment.dueDate} onChange={(e) => setNewPayment(prev => ({...prev, dueDate: e.target.value}))} />
                </div>
                <div>
                  ...
                  <Select value={newPayment.method} onValueChange={(value) => setNewPayment(prev => ({...prev, method: value}))}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="credito_recorrente">Crédito Recorrente</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
            ...
            <Card key={payment.id} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={getStatusColor(payment.status || 'pendente')}>{getStatusLabel(payment.status || 'pendente')}</Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-lg">R$ {Number(payment.valor).toLocaleString()}</span>
                  </div>
                  {payment.data_vencimento && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">Vencimento:</span>
                      <span className="text-sm">{new Date(payment.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR')}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center">
                  ...
                  {payment.status === "pendente" && payment.data_vencimento && getDaysOverdue(payment.data_vencimento) > 0 && (
                    <div className="bg-red-50 dark:bg-red-950/30 p-2 rounded border border-red-200">
                      <p className="text-xs text-red-800 dark:text-red-300">Atrasado há {getDaysOverdue(payment.data_vencimento)} dias</p>
                    </div>
                  )}
                  {payment.status === "pago" && payment.data_pagamento && (
                    <div className="bg-green-50 dark:bg-green-950/30 p-2 rounded border border-green-200">
                      <p className="text-xs text-green-800 dark:text-green-300">Pago em {new Date(payment.data_pagamento + 'T00:00:00').toLocaleDateString('pt-BR')}</p>
                    </div>
                  )}
                  <div className="flex space-x-2 mt-4">
                    {payment.status !== "pago" && (
                      <Button size="sm" variant="outline" onClick={() => handleMarkAsPaid(payment.id)}>Marcar Pago</Button>
                    )}
                  </div>
                </div>
              </CardContent>
      ...
      {payments.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-muted-foreground mt-1">Crie sua primeira cobrança ou gere cobranças em lote</p>
        </div>
      )}
    </div>
  );
