...
interface EquipmentCardProps {
  ...
  onUpdateEquipment: (id: string, data: Partial<EquipmentItem>) => void;
}

export function EquipmentCard({ equipment, onUpdateEquipment }: EquipmentCardProps) {
  const getStatusInfo = (status: string) => {
    switch(status) {
      case "working": return { color: "bg-green-100 text-green-800", icon: CheckCircle, label: "Ativo" };
      case "maintenance": return { color: "bg-yellow-100 text-yellow-800", icon: Wrench, label: "Manutenção" };
      case "broken": return { color: "bg-red-100 text-red-800", icon: AlertTriangle, label: "Quebrado" };
      default: return { color: "bg-gray-100 text-gray-800", icon: CheckCircle, label: "Desconhecido" };
    }
  };
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Badge className={getCategoryColor(equipment.type)}>{equipment.type}</Badge>
