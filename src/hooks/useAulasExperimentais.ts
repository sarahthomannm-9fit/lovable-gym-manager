...
export function useAulasExperimentais() {
  ...
  const fetchAulasExperimentais = useCallback(async () => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível carregar as aulas experimentais",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const addAulaExperimental = useCallback(async (aula: AulaExperimentalInput) => {
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Aula experimental agendada com sucesso!",
      });
      return data;
    } catch (error) {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível agendar a aula experimental",
        variant: "destructive",
      });
      throw error;
    }
  ...
  const updateAulaExperimental = useCallback(async (id: string, updates: Partial<AulaExperimentalInput>) => {
    try {
      ...
      setAulasExperimentais(prev => 
        prev.map(a => a.id === id ? (data as AulaExperimental) : a)
      );
      toast({
        title: "Sucesso",
        description: "Aula experimental atualizada!",
      });
      return data;
    } catch (error) {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível atualizar a aula experimental",
        variant: "destructive",
      });
      throw error;
    }
  ...
  const converterParaAluno = useCallback(async (id: string, planoId: string) => {
    try {
      const { data, error } = await supabase
        .from('aulas_experimentais')
        .update({
      ...
      toast({
        title: "Sucesso",
        description: "Lead convertido em aluno com sucesso!",
      });
      return data;
    } catch (error) {
  ...
  const marcarNaoCompareceu = useCallback(async (id: string) => {
    return updateAulaExperimental(id, { status: 'nao_compareceu' });
  }, [updateAulaExperimental]);

  const marcarRealizada = useCallback(async (id: string) => {
    return updateAulaExperimental(id, { status: 'realizada' });
  }, [updateAulaExperimental]);
  ...
  const getMetricasFunil = useCallback(() => {
    ...
    const taxaComparecimento = realizadas + convertidas + naoConvertidas > 0 
      ? ((realizadas + convertidas + naoConvertidas) / (realizadas + convertidas + naoConvertidas + naoCompareceram)) * 100 
      : 0;
    
    const taxaConversao = realizadas + convertidas + naoConvertidas > 0
      ? (convertidas / (realizadas + convertidas + naoConvertidas)) * 100
      : 0;
  ...
  const getByFonte = useCallback(() => {
    ...
    aulasExperimentais.forEach(a => {
      const fonte = a.fonte || 'outro';
  ...
  return {
    ...
    getMetricasFunil,
    getByFonte,
    refetch: fetchAulasExperimentais,
  };
}
